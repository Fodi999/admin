...
export async function publishProduct(token: string, id: string): Promise<Product> {
  const res = await fetch(`${API_BASE}/api/admin/catalog/products/${id}/publish`, {
    method: 'POST',
    headers: authHeaders(token),
  });
  if (!res.ok) {
...
export async function unpublishProduct(token: string, id: string): Promise<Product> {
  const res = await fetch(`${API_BASE}/api/admin/catalog/products/${id}/unpublish`, {
    method: 'POST',
    headers: authHeaders(token),
  });
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`Failed to unpublish: ${text}`);
  }
  return res.json();
}
 ...
 * 
 * Returns overall catalog health score and missing fields per product.
 */
export async function getDataQuality(token: string): Promise<DataQualitySummary> {
  const res = await fetch(`${API_BASE}/api/admin/catalog/states/data-quality`, {
    headers: authHeaders(token),
  });
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Data quality failed: ${text} (${res.status})`);
  }
  return res.json();
}
